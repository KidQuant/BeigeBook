...
def analyze_all():
    ...
    for year, month, region in gen(skip=True):
        ...
        print(
            f"score: {f_score[0].labels[0].score*(-1,1)[f_score[0].labels[0].value=='POSITIVE']}"
        )
        outfile.write(
            f"{year},{month:02d},{region},{v_score['compound']},{v_score['neg']},{v_score['neu']},{v_score['pos']},{t_score.subjectivity},{t_score.polarity},{f_score[0].labels[0].score*(-1,1)[f_score[0].labels[0].value=='POSITIVE']}\n"
        )
    outfile.close()
