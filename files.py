...
def filesizes():
    ...
    for (
        ...
        if os.path.exists(filename):
            errorfile.write(
                f"{year}, {month:02d}, {region}, {os.path.getsize(filename)}\n"
            )
        errorfile.close()
...
def analyze_missing(printing=True, writing=True):
    df = pd.read_csv("out/csv/missings.csv")
    empty = (
        df.groupby(["year", "month"])
        .aggregate(count=("region", "size"))
        .query("count == 13")
    )
    if printing:
        print(empty)
...
def analyze_filesize(printing=True, writing=True):
    ...
    if writing:
        smallfiles.to_csv(
            "out/csv/smallfiles.csv", columns=["year", "month", "region"], index=False
        )
